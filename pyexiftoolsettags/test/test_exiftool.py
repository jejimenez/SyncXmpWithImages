...
class TestExifTool(unittest.TestCase):
    def setUp(self):
        self.et = exiftool.ExifTool(executable_="/usr/local/Cellar/exiftool/10.20/bin/exiftool", addedargs=["-overwrite_original"])
    def tearDown(self):
        if hasattr(self, "et"):
            self.et.terminate()
    ...
    def test_set_keywords(self):
        ...
        for d in expected_data:
            ...
            with self.et:
                ...
                kwtag0 = self.et.get_tag("XMP:Subject", f_mod)
                kwrest = d["Keywords"][1:]
                self.et.set_keywords(exiftool.KW_REMOVE, kwrest, f_mod)            
                kwtag1 = self.et.get_tag("XMP:Subject", f_mod)
                self.et.set_keywords(exiftool.KW_ADD, kw_to_add, f_mod)            
                kwtag2 = self.et.get_tag("XMP:Subject", f_mod)
            os.remove(f_mod)
